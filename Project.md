...
- `click_keep_ms` — how long the originator keeps unacked clicks to piggyback on later Orders (e.g. 10000)

Play always goes through a relay room (localhost server + your own room is fine). After Join, Start means **ready**. A successful `StartRoom` snapshots that room's seated `player_ids` and join-order `seats` (`u8`, 0, 1, 2, …). The seat byte is the engine `PlayerId` (not a hash of the login string). When every seated player has started, the server sends a UDP Kickoff; each client waits `remaining_ms - RTT/2` then arms the sim pacer (`T0`). A click is sent immediately with the originator's **Actual Tick** (wall lattice from `T0`, not sim `GetTick()`). Idle Actual Ticks still emit an empty Order (no unit ids). Empties reuse the last click `order_id` (0 before any click); a consecutive click (`order_id = last + 1`) or a later empty with that same id fills missing Actual Ticks since that click. Every Order carries an ack vector `(seat, last_click_order_id)` for the seated set. The host caches clicks, bounces a per-recipient packet with fully-acked watermarks, and piggybacks clicks a dest is missing. Originators keep clicks for `click_keep_ms` and piggyback still-unacked ones on the next empty or click. Every client schedules activation at `actual_tick + future_tick_distance` and pathfinds only then, sorted by seat then order id. `StepForward` waits until every seated player has a command (empty or click) for `actual_tick = GetTick() - future_tick_distance`; empties keep going out on Actual Tick while the sim is locked. UDP orders also carry a `(hash_tick, state_hash)` pair of committed `BattleState` after `StepForward` (not Actual Tick, not yaw, not the scheduled-order cache). Peers compare hashes for the same sim tick to detect desync (self-bounce uses seat). A missing or invalid Networking.json logs an error and does not start comms.
